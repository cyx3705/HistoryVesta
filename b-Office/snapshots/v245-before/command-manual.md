...
> 需要更新时，请在程序控制台执行 `command.manual file=b-Office/meta/命令手册.md apply=true`。
...
### `ProjectPulse.Largest`

列出一个 OHS 工作树中最大的文件

- 来源：`module:ProjectPulse`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ProjectPulse_Largest`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `name` | `string` | 是 | - | - | 已登记工作树名称，例如 2026-020-OneHistoryStudio |
...
```text
ProjectPulse.Largest name=文本 limit=1 minMb=1.5 includeGenerated=true
```
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ProjectPulse_Recent`
...
```text
ProjectPulse.Recent name=文本 days=1 limit=1 includeGenerated=true
```

### `ProjectPulse.Summary`

汇总一个 OHS 工作树的文件数量、体积、格式和一级目录热点

- 来源：`module:ProjectPulse`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ProjectPulse_Summary`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `name` | `string` | 是 | - | - | 已登记工作树名称，例如 2026-020-OneHistoryStudio |
| `includeGenerated` | `bool` | 否 | false | - | 是否包含 bin、obj、.vs、node_modules 等生成目录 |
| `top` | `int` | 否 | 10 | - | 最多返回多少项格式和一级目录统计，范围 1~50 |

```text
ProjectPulse.Summary name=文本 includeGenerated=true top=1
```
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ToolKit_Base64Encode`
...
通过 OHS 最新 MCP 工具目录调用一个工具，不依赖 Codex 当前任务的旧快照

- 来源：`module:ToolRelay`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ToolRelay_Call`
...
```text
ToolRelay.Call name=文本 argumentsJson=文本
```

### `ToolRelay.Describe`

实时查看一个当前可见 MCP 工具的描述和 JSON Schema

- 来源：`module:ToolRelay`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ToolRelay_Describe`
...
```text
ToolRelay.Describe name=文本
```

### `ToolRelay.List`

实时列出 OHS 当前 MCP 策略下可见的工具

- 来源：`module:ToolRelay`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略可见，工具名 `ToolRelay_List`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `filter` | `string` | 否 | - | - | 按工具名或描述包含匹配，空字符串不过滤 |
| `modulesOnly` | `bool` | 否 | true | - | true 只返回模块工具，false 返回全部可见工具 |

```text
ToolRelay.List filter=文本 modulesOnly=true
```
