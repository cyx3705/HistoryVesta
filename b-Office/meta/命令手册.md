...
| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `url` | `string` | 是 | - | - | https://host:port/ |
| `fingerprint` | `string` | 是 | - | - | 服务器证书 SHA-256 指纹 |
| `apply` | `bool` | 否 | false | - | false 预览；true 保存到当前设备 |

### `conn.forget`

清除当前设备保存的 token 和服务器证书 pin

- 来源：`app:frontend`
- 安全：本地二次确认
- UI 线程：否
- MCP：`dangerous`，当前策略隐藏，工具名 `conn_forget`
...
| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `code` | `string` | 是 | - | - | 一次性配对码（回显自动掩码） |
| `fingerprint` | `string` | 是 | - | - | 服务器证书 SHA-256 指纹 |
| `name` | `string` | 是 | - | - | 设备显示名称 |

### `conn.reconnect`

使用当前本机 profile 重连服务器
...
| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `mode` | `string` | 是 | - | server / client | 当前设备角色 |
| `apply` | `bool` | 否 | false | - | false 预览；true 保存到当前设备 |
...
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `conn_status`
...
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `res_list`
