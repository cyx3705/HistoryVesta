# V2.4.5 structuredContent 客户端行为实验
...
原方案设计是独立探针 MCP 服务 + 双组 canary。实际执行中发现
...
> 引用前应复测。原方案设想的「移除 content[1] 省上下文」在 Claude Code 侧
> **已无必要**——客户端自己完成了择一。
...
| 「移除 `content[1]`」议题**收益归零** | 客户端已自行择一;移除只会破坏旧网关兼容与规范 SHOULD,无任何上下文收益。建议永久搁置,除非未来客户端行为改变 |
