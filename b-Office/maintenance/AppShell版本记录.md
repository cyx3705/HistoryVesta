...
| 版本 | 日期 | 里程碑 | 说明 |
|---|---|---|---|
| 3.0.3（候选） | 2026-07-29 | 主工作区比例收口 | 将左右或上下侧栏总上限从 80% 收紧到 50%，确保中央主工作区至少占对应轴的一半；加载历史布局时主动把同侧多个 `LayoutAnchorablePane` 合并为一个标签组，修复旧模块侧栏继续挤占中央区的问题。公开 API 不变。完整记录见 `AppShell_3.0.3_主工作区比例收口.md` |
