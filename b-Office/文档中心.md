...
`OneHistoryStudio/` 共同构成有效基线；`history/` 只用于明确的版本追溯，不得作为新开发的默认输入。
...
├─ OneHistoryStudio/    供模块及其他集成项目消费的稳定合同
...
`OneHistoryStudio/` 是其他项目开发 OHS 模块或接入 OHS 服务时可以直接消费的文档包，只保留当前有效合同：

- [模块开发手册](./OneHistoryStudio/模块开发手册.md)：模块契约、装载、热重载、窗口和工具同步。
- [命令手册](./OneHistoryStudio/命令手册.md)：由运行时注册表生成的命令、参数和 MCP 投影。
- [MCP 接入与安全](./OneHistoryStudio/MCP接入与安全.md)：网关、策略、鉴权、确认和审计合同。

模块项目以 `OneHistoryStudio/` 为消费入口，不需要读取 OHS 的 `history/` 或内部发布记录。命令手册仍由运行时生成，
禁止手工维护命令条目。
...
| `OneHistoryStudio/模块开发手册.md` | `docs/模块开发手册.md` |
| `OneHistoryStudio/命令手册.md` | `docs/命令手册.md` |
| `OneHistoryStudio/MCP接入与安全.md` | `docs/MCP接入与安全.md` |

## 历史边界

`history/` 保存已交付版本文档，没有独立 README。默认不得列举、全文搜索、批量读取或概括该目录；只有用户
明确指定追溯某个版本时，才读取最小必要文件，并以当前源码、运行时、`current/` 和 `OneHistoryStudio/` 复核结论。
