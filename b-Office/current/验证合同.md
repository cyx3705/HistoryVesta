...
- 3.0.3 冻结门禁不得因 runner 差异而删除或弱化测试；3.1.1 至 3.1.4 新增合同不得绕过冻结门禁。
...
| VERIFY-PACKAGE | 包合同或打包脚本修改 | `Publish-AppShell.ps1 -Version 3.1.4`（不带 `-Publish`） | 漏洞审计、四包、符号、Demo、PackageSmoke、文档哈希通过 |
...
- 3.1.4 控制台冒烟还需在停靠、浮动、最大化三种状态下拖窄/拖宽，确认既有长文本即时重排且无水平条；
