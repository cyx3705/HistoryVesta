...
- 3.0.3 冻结门禁不得因 runner 差异而删除或弱化测试；3.1.1 至 3.1.10 新增合同不得绕过冻结门禁。
...
| VERIFY-HOST | 宿主、消费文档或部署脚本修改 | `Publish-AppShellHost.ps1 -Version 3.1.10` | `AppShell.exe` 为 3.1.10.0，宿主、全部清单文档和 SHA-256 完整 |
| VERIFY-RELEASE | 正式宿主部署前 | 宿主候选审查、人工批准后 `-DeployToZ` | `b-Publish/current` 与 `z-Package-AppShell` 文件和 SHA-256 一致 |
| VERIFY-PACKAGE | 兼容包合同或历史打包脚本修改 | `Publish-AppShell.ps1 -Version 3.1.10`（不带 `-Publish`） | 漏洞审计、四包、符号、PackageSmoke 和文档哈希通过 |
...
- 2026-08-07 本地（3.1.8 “轻松指令”控制台）：锁定还原通过；Debug/Release 构建均为 0 warning、0 error，
  ...
  `FrontendFocusConsoleRaisesTheWindowAndRefocusesExistingConsole` 因人工启动的 AppShell 前后台进程争夺
