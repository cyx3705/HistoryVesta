...
- 3.0.3 冻结门禁不得因 runner 差异而删除或弱化测试；3.1.1 至 3.1.9 新增合同不得绕过冻结门禁。
...
| VERIFY-HOST | 宿主、消费文档或部署脚本修改 | `Publish-AppShellHost.ps1 -Version 3.1.9` | `AppShell.exe` 为 3.1.9.0，宿主、全部清单文档和 SHA-256 完整 |
| VERIFY-RELEASE | 正式宿主部署前 | 宿主候选审查、人工批准后 `-DeployToZ` | `b-Publish/current` 与 `z-Package-AppShell` 文件和 SHA-256 一致 |
| VERIFY-PACKAGE | 兼容包合同或历史打包脚本修改 | `Publish-AppShell.ps1 -Version 3.1.9`（不带 `-Publish`） | 漏洞审计、四包、符号、PackageSmoke 和文档哈希通过 |
...
- 3.1.8 控制台冒烟需确认只有控制台 `win.max` 聚焦态从输入框上方向上弹出候选且输入焦点不丢失；命令、参数名、
  ...
  清空后恢复全部结果。两种模式均不得最大化错误页面或丢失输入焦点；浅色/深色、停靠/浮动/聚焦和窄宽度不得遮挡或漏色。
