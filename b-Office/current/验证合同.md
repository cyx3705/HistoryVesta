...
- 3.0.3 冻结门禁不得因 runner 差异而删除或弱化测试；3.1.1 至 3.3.0 新增合同不得绕过冻结门禁。
...
| 编号 | 适用时机 | 命令或步骤 | 通过标准 |
| --- | --- | --- | --- |
| VERIFY-FAST | 文档或公开面相关修改 | `b-Code-HistoryVulcan/eng/Test-ProjectContract.ps1 -Instantiation` 与 `Assert-PublicApiBaseline.ps1` | 合同无错误，Unshipped 只包含 3.3.0 已批准增量 |
...
| VERIFY-HOST | 宿主、消费文档或部署脚本修改 | `Publish-HistoryVulcanHost.ps1 -Version 3.3.0` | `HistoryVulcan.exe` 为 3.3.0.0，宿主、全部清单文档和 SHA-256 完整 |
| VERIFY-RELEASE | 正式宿主部署前 | 宿主候选审查、人工批准后 `-DeployToZ` | `b-Publish/current` 与正式快照（3.2.0 起 `z-HistoryVulcan`）文件和 SHA-256 一致 |
| VERIFY-PACKAGE | 兼容包合同或历史打包脚本修改 | `Publish-AppShell.ps1 -Version <历史版本>`（不带 `-Publish`） | 历史兼容包、符号、PackageSmoke 和文档哈希通过 |
