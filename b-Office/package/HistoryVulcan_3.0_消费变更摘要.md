...
- HistoryVulcan 自身以 `host/HistoryVulcan.exe` 部署；当前稳定消费快照为 3.1.9（旧名 AppShell），3.2.0 尚未部署且不生成 NuGet 包。
...
- 3.2.0 将产品从 AppShell 改名为 HistoryVulcan：包 ID 改为 `OneHistory.HistoryVulcan.*`，命名空间改为
  ...
  需要把包引用和 `using` 命名空间整体换为新名。正式快照入口随之迁移到 `z-Package-HistoryVulcan`。
