...
适用版本：AppShell 3.1.10 候选（包含 3.0.3 冻结基线、3.1.1 命令收口、3.1.2 命令目录修复、3.1.3 浮窗几何收口、3.1.5 质量更新、3.1.6 异常修复、3.1.7 UI 风格合同、3.1.8 轻松指令过渡实现、3.1.9 模块宿主合同和 3.1.10 内部命令目录内聚重构）。

本文只记录会影响消费应用、模块作者和部署者的变化；源码施工、冻结审查、完整测试证据和发布操作不属于本文。

## 部署与引用方式

- AppShell 自身以 `host/AppShell.exe` 部署；当前稳定消费快照为 3.1.9，3.1.10 尚未部署且不生成 NuGet 包。
...
- AppShell 的 `ModulesView` 是唯一模块管理页面。Janus 只保留 `StandardWindowIds.Modules` 的位置与消费配置，
