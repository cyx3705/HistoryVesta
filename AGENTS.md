...
本文件适用于整个仓库。**当前源码候选为 3.5.0，正式 Z 部署仍为 3.3.2**；`v3.0.3` 是 V3 历史冻结标签，
只对 3.0.x 维护分支有效，不约束当前 3.5.x 开发线。进入项目后先确认现行合同和修改边界，
再按任务读取最小必要上下文。
...
- 当前开发线是 **3.3.x**，正式快照 3.3.2。新增公开 API 允许，但必须进入对应版本的
...
- **门禁只验证本仓库自身**：解决方案与 CI 不得出现指向兄弟项目工作树的跨仓库
  `ProjectReference`。依赖其他项目实现的联调用例归各自仓库（DEC-023）。
  各 `2026-0xx-*` 目录是同一个仓库的不同 worktree 分支，本地存在不代表 CI 上存在。
