...
这是其他项目和 AI 接入 AppShell 时应优先读取的首要入口。当前正式版本为 **3.0.2**，正式包位于同级 `feed/`，当前版本消费合同位于同级 `docs/`；不要扫描发布归档、历史版本或框架源码来推断用法。
...
```xml
<PackageReference Include="OneHistory.AppShell.Shell" Version="3.0.2" />
```

无窗服务宿主只直接引用 ServiceHost；`Core` 和 `Services` 会传递进入：

```xml
<PackageReference Include="OneHistory.AppShell.ServiceHost" Version="3.0.2" />
```
