...
这是其他项目和 AI 接入 AppShell 时应优先读取的首要入口。当前正式版本为 **3.1.2**，正式包位于同级 `feed/`，当前版本消费合同位于同级 `docs/`；不要扫描发布归档、历史版本或框架源码来推断用法。
...
```xml
<PackageReference Include="OneHistory.AppShell.Shell" Version="3.1.2" />
```

无窗服务宿主只直接引用 ServiceHost；`Core` 和 `Services` 会传递进入：

```xml
<PackageReference Include="OneHistory.AppShell.ServiceHost" Version="3.1.2" />
```
...
| `OneHistory.AppShell.Services` | 设置、日志、文件状态、模块宿主、MCP/Web 服务实现 |
...
- AppShell 的 `ModulesView` 是唯一模块管理页面；OHS 等消费方只配置 `StandardWindowIds.Modules` 的位置，
  不要复制第二套模块管理 UI。3.1.2 保留现有模块目录抽象，不执行全局 `OneHistory-Projects\\*\\z-*` 扫描。
- 资源浏览与文件操作不属于 AppShell；`IWorkspaceService`、`ResourceView` 和 `res.*` 已删除，未来由独立模块提供。
- 控制台检索与操作使用 `log.level/source/keyword/mute/autoscroll/clear/export/copy/focus`；`cls` 仅为 `log.clear` 兼容别名。
