...
本手册给出稳定公开 API 的常用入口和 3.0.x 框架基础命令。完整签名以包内 `lib/<TFM>/AppShell.*.xml` 和对应版本的 `PublicAPI.Shipped.txt` 为准；最终命令集合以应用运行时的 `command.list`、`command.show` 和 `command.manual` 为准。
...
config.ToolWindows.Add(new ToolWindowDescriptor
{
    ...
    DefaultRatio = 1,
    ContentFactory = () => new WorkspaceView(),
});
...
| `app.exit` | 正常关闭桌面应用 |
| `app.about` | 显示应用身份与版本 |
| `app.get [key]` | 读取一个或全部设置 |
| `app.set key= value=` | 写设置 |
| `app.opendata` | 打开应用数据目录 |
| `log.level level=trace|debug|info|warn|error` | 设置控制台日志级别 |
...
| `win.dock name= pos=left|right|top|bottom|center|tab [target=] [ratio=]` | 停靠窗口；Center 进入主文档区，tab 需要目标；命令集只允许 Center |
| `win.ratio name= value=` | 设置窗口占主窗体比例 0 到 1 |
...
| 命令 | 用途 / 关键参数 |
|---|---|
| `command.list [mcp=policy]` | 查看权威命令目录和 MCP 状态 |
| `command.show name=` | 查看单条命令完整元数据 |
| `command.domains` | 按域统计命令 |
| `command.manual [file=] [apply=false]` | 生成运行时命令手册；apply 由宿主控制落位 |
...
| `mcp.port [value]`、`mcp.token [value]`、`mcp.policy [value]`、`mcp.confirm [value]`、`mcp.autostart [value]` | 查看或修改 MCP 配置 |
...
| `web.confirm mode=` | 设置 Web 确认模式 |
| `svc.status` | 查看服务、MCP、Web 和模块状态 |
| `svc.stop`、`svc.restart` | 请求停止或重启服务 |
| `svc.autostart mode=status|on|off` | 查看或修改登录自启 |
