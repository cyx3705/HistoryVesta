...
适用版本：AppShell 3.1.7（包含 3.0.3 冻结基线、3.1.1 命令收口、3.1.2 命令目录修复、3.1.3 浮窗几何收口、3.1.5 质量更新、3.1.6 异常修复和 3.1.7 UI 风格合同）。

本文只记录会影响消费应用、模块作者和部署者的变化；源码施工、冻结审查、完整测试证据和发布操作不属于本文。

## 部署与引用方式

- AppShell 自身以 `host/AppShell.exe` 部署；3.1.7 当前快照不生成 NuGet 包。
