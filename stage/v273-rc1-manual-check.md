...
- 来源：`framework:frontend`
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `app_about`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_endpoint`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_forget`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_pair`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_reconnect`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_role`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `conn_status`
...
```text
correction.list name=db.query limit=20
```
...
```text
correction.propose name=db.query claim="返回整张表" correction="按 limit 和 page 分页返回"
```
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `db_export`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `db_insert`
...
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `github_accounts`
...
- 安全：本地二次确认
- UI 线程：否
- MCP：`dangerous`，当前策略隐藏，工具名 `github_identity`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `name` | `string` | 是 | - | - | Git user.name |
| `email` | `string` | 是 | - | - | Git user.email |
| `scope` | `string` | 否 | repository | repository / global | 写入作用域 |
| `apply` | `bool` | 否 | false | - | false 预览；true 确认后修改 |

### `github.login`

在服务器本机会话中启动 GCM GitHub 登录

- 来源：`app`
- 安全：本地二次确认
- UI 线程：否
- MCP：`dangerous`，当前策略隐藏，工具名 `github_login`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `account` | `string` | 否 | - | - | 可选 GitHub 用户名 |

### `github.logout`

预览或注销服务器 GCM GitHub HTTPS 凭据账号

- 来源：`app`
- 安全：本地二次确认
- UI 线程：否
- MCP：`dangerous`，当前策略隐藏，工具名 `github_logout`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `account` | `string` | 是 | - | - | GCM 账号 |
| `apply` | `bool` | 否 | false | - | false 预览；true 确认后注销 |

### `github.remote`

预览或分别修改服务器 origin fetch/push URL

- 来源：`app`
- 安全：本地二次确认
- UI 线程：否
- MCP：`dangerous`，当前策略隐藏，工具名 `github_remote`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `fetch` | `string` | 否 | - | - | 新的 fetch URL |
| `push` | `string` | 否 | - | - | 新的 push URL |
| `apply` | `bool` | 否 | false | - | false 预览；true 确认后修改 |

### `github.status`

查看服务器 Git 作者、GitHub 凭据、origin 与 SSH 状态

- 来源：`app`
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `github_status`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `refresh` | `bool` | 否 | false | - | 重新读取全部本机事实 |

### `github.test`

只读检测服务器 GitHub SSH/HTTPS 连接，不执行 push

- 来源：`app`
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `github_test`

| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `transport` | `string` | 否 | auto | auto / ssh / https | 检测通道 |
| `timeout` | `int` | 否 | 15 | - | 超时秒数 |
...
```text
incident.list name=db.query limit=20
```
...
```text
incident.record name=db.query symptom=误解分页范围 expected=只读当前页 actual=请求整表
```
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `layout_load`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `layout_reset`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `layout_save`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `log_level`
...
```text
mcp.desc name=db.query text="查询数据表并返回分页结果" reason=人工修订
```
...
```text
mcp.parse command=db.query args="{\"table\":\"users\",\"limit\":20}" exec=true
```
...
| 参数 | 类型 | 必填 | 默认值 | 允许值 | 说明 |
|---|---|---|---|---|---|
| `name` | `string` | 否 | - | - | 指令名或工具名(如 db.query / db_query);省略列出全部 |

```text
mcp.schema name=db.query
```
...
- 安全：普通
- UI 线程：否
- MCP：`readonly`，当前策略可见，工具名 `panel_list`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `panel_reload`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `panel_set`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `panel_show`
...
| `submodules` | `bool` | 否 | false | - | V2.3.1 兼容参数：true=both，false=parent；target 存在时忽略 |
...
```text
prompt.history name=db.query limit=20
```
...
```text
prompt.propose name=db.query text="查询数据表并返回分页结果" reason=澄清分页语义
```
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_delete`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_list`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_mkdir`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_open`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_rename`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_reveal`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `res_root`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_dock`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_float`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_hide`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_max`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_ratio`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_reset`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_restore`
...
- 安全：普通
- UI 线程：否
- MCP：`standard`，当前策略隐藏，工具名 `win_show`
