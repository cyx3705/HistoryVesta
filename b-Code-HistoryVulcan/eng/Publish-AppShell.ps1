...
$ReuseDocumentTemplate = Join-Path $ReleaseDocumentRoot "HistoryVulcan.reuse.template.md"
...
function Get-VulcanVersionProperties {
    $project = Join-Path $ComponentRoot "src\App\App.csproj"
    $output = & dotnet msbuild $project -nologo `
        -getProperty:VulcanVersion -getProperty:FileVersion
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to evaluate the HistoryVulcan version source"
    }
    ...
    try {
        ...
        throw "HistoryVulcan version evaluation returned invalid JSON: $($output -join ' ')"
    }
}

$VersionProperties = Get-VulcanVersionProperties
...
if ([string]::IsNullOrWhiteSpace($SourceVersion) -or
    ...
    throw "VulcanVersion or FileVersion evaluated to an empty value"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Unable to inspect HistoryVulcan release inputs with git status"
}
...
function Install-ZSnapshot {
    param(
        ...
        [string]$ReuseFileName = 'AppShell.reuse.md',
        [string[]]$PackageIds = @(
    ...
    foreach ($requiredName in @($ReuseFileName, "manifest.json", "README.md", "SHA256SUMS")) {
    ...
    $expectedPackageNames = @($PackageIds | ForEach-Object { "$_.$Version.nupkg" })
...
function Publish-CurrentSnapshot {
    ...
    $snapshotReuse = Join-Path $snapshot "HistoryVulcan.reuse.md"
    ...
    Install-ZSnapshot -SourceDirectory $snapshot -ReuseFileName 'HistoryVulcan.reuse.md' -PackageIds @(
        'OneHistory.HistoryVulcan.Core',
        'OneHistory.HistoryVulcan.Services',
        'OneHistory.HistoryVulcan.Shell',
        'OneHistory.HistoryVulcan.ServiceHost'
    )
...
function Assert-CurrentCandidate {
    ...
    $reuseDocument = Join-Path $StageRoot "HistoryVulcan.reuse.md"
    ...
    if ([int]$manifest.schemaVersion -ne 1 -or [string]$manifest.product -ne "HistoryVulcan" -or
    ...
    $expectedPayloads["HistoryVulcan.reuse.md"] = [ordered]@{
...
function Publish-CurrentCandidate {
    ...
    Write-Host "Promoted the reviewed HistoryVulcan $Version candidate without rebuilding packages"
...
> Package version: $VirtualVersion. The document body comes from the HistoryVulcan $SourceVersion consumer-contract source and validates only the release-generation pipeline. Compatibility with $VirtualVersion is not asserted.
...
$publishMutex = [Threading.Mutex]::new($false, "Local\OneHistory.HistoryVulcan.Publish.$mutexHash")
$mutexAcquired = $false
try {
...
if (-not $mutexAcquired) {
    ...
    throw "Another HistoryVulcan publish is already running for $ComponentRoot"
}
...
$NugetCache = Join-Path ([IO.Path]::GetTempPath()) ("historyvulcan-release-cache-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    $ReuseDocumentPath = Join-Path $StageRoot "HistoryVulcan.reuse.md"
    ...
    Invoke-Dotnet @("restore", "HistoryVulcan.sln", "--locked-mode", $artifactsProperty)
    Invoke-Dotnet @("build", "HistoryVulcan.sln", "-c", "Debug", "--no-restore", $artifactsProperty)
    Invoke-Dotnet @("test", "..\b-Code-Tests\HistoryVulcan.Tests\HistoryVulcan.Tests.csproj", "-c", "Debug",
        "--no-build", "--no-restore", $artifactsProperty)
    Invoke-Dotnet @("build", "HistoryVulcan.sln", "-c", "Release", "--no-restore", $artifactsProperty)
    Invoke-Dotnet @("test", "..\b-Code-Tests\HistoryVulcan.Tests\HistoryVulcan.Tests.csproj", "-c", "Release",
        "--no-build", "--no-restore", $artifactsProperty)

    $auditPath = Join-Path $StageRoot "vulnerability-audit.json"
    $auditOutput = & dotnet list HistoryVulcan.sln package --vulnerable --include-transitive --format json
    if ($LASTEXITCODE -ne 0) {
    ...
    $packProjects = @(
        "src\HistoryVulcan.Core\HistoryVulcan.Core.csproj",
        "src\HistoryVulcan.Services\HistoryVulcan.Services.csproj",
        "src\HistoryVulcan.Shell\HistoryVulcan.Shell.csproj",
        "src\HistoryVulcan.ServiceHost\HistoryVulcan.ServiceHost.csproj"
    )
    ...
    if (Get-ChildItem -LiteralPath $PackagesDir -Filter "HistoryVulcan.$Version.nupkg" -ErrorAction SilentlyContinue) {
...
<configuration>
  <packageSources>
    <clear />
    <add key="HistoryVulcan staging" value="$escapedPackagesDir" />
...
"@
    ...
    $demoExe = Join-Path $DemoDir "HistoryVulcan.exe"
    Assert-File $demoExe
    ...
    $demoZip = Join-Path $PackagesDir "HistoryVulcan-Demo-$Version-win-x64-framework-dependent.zip"
    ...
    $checksumLines = @(
        ...
        "$reuseHash  HistoryVulcan.reuse.md"
    )
    $checksumPath = Join-Path $StageRoot "$Version.sha256"
    ...
    Write-Host "Prepared HistoryVulcan $Version current candidate at $StageRoot"
    $PipelineSucceeded = $true
}
finally {
    & dotnet build-server shutdown | Out-Null
    $env:NUGET_PACKAGES = $PreviousNugetPackages
    $workspaceRestoreOutput = & dotnet restore HistoryVulcan.sln --locked-mode --force-evaluate 2>&1
