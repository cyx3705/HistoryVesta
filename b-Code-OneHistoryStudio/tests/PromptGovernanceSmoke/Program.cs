...
try
{
    ...
    var manualPreview = await bus.ExecuteAsync(
        "command.manual file=b-Code-OneHistoryStudio/tests/manual-preview.md apply=false", "UI");
    ...
    True(!File.Exists(Path.Combine(Environment.CurrentDirectory,
        "b-Code-OneHistoryStudio", "tests", "manual-preview.md")),
