...
const string parentBranch = "2026-232-Parent";
const string noChildBranch = "2026-233-NoChild";
const string childBranch = "child-main";
const string childRelative = "child repo 中文";
...
static int CountGridRows(XElement group)
...
{
    Ensure(await GitRunner.RunAsync(repository, ["config", "user.name", "V232 Smoke"]), "git user name");
    Ensure(await GitRunner.RunAsync(repository, ["config", "user.email", "v232@example.invalid"]),
        "git user email");
}

static string Subject(string repository) => FirstLine(Run(repository, ["log", "-1", "--format=%s"]));
static string Sha(string repository) => FirstLine(Run(repository, ["rev-parse", "HEAD"]));
...
static string Run(string repository, IReadOnlyList<string> arguments)
{
