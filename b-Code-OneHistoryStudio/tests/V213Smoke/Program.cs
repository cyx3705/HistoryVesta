...
static byte[] OutsideManagedBytes(byte[] bytes)
{
...
static byte[] SnapshotOptional(string path) => File.Exists(path) ? File.ReadAllBytes(path) : [];
...
static void BytesEqual(byte[] expected, byte[] actual, string name)
