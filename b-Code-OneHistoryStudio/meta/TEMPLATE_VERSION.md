...
## 本应用基线登记(OneHistoryStudio)

- **派生应用**:OneHistoryStudio
- **跟随基线**:0.4.4 ｜ **未分叉**(2026-07-24 V2.4 前反哺后与模板三目录逐文件哈希比对 0 差异,共 68 文件)
  > 0.4.4 反哺:MCP 服务 / 模块注册器 / 提示词治理三大件由本应用上抛为框架自带能力;
  > 本应用装配点随之瘦身——不再自建这三件,只登记应用专有的 MCP 只读白名单(见 AppMcpPolicy)
  > 并把 HistoryRecorder 作为 IMcpAuditLog 接入框架。明细见模板 TEMPLATE_VERSION.md 的 0.4.4 行。
  > 纪律备注:V2.1.2~V2.1.5 期间五个框架文件曾未经登记先行修改(实际分叉),V2.1.6 Q16-M1
  > 审阅后回灌转正为 0.4.3。今后框架改动仍应先走演进手册上报,不欠账。
- **派生日期**:2026-07-14(V2-M0)
- **反哺记录**:0.4.2 的两处修正(控制台多行日志逐行拆分+Pixel 滚动;CommandRegistry.Unregister)
  由本应用 V2-M2/M3 开发中产生,经用户批准于 2026-07-15 回灌模板
- **应用修复快照**:2.1.0-M1.5(2026-07-15);仅修改 App 自由区的 Git 参数与项目路径边界，
  AppShell.Core / Services / Shell 基线未改，不影响 V21-M2 MCP 网关推进
- **交付记录**:V2.0(2026-07-15,V2-M0~M5)→ V2.0.1(Meta 文件窗口)→ V2.1.0
  (2026-07-16,指令总线 MCP 化 + MD-08 面板自动发现;跟随基线 0.4.2 未分叉,交付时哈希复核 0 差异)
  → V2.1.1(2026-07-16,MCP 工具管理页 + 模块管理页 + mcp.desc 提示词覆盖;零框架改动)
  → **V2.1.2(2026-07-17,应用身份单一真值 + MCP提示词修订/提案/勘误/事故治理;零框架改动)**
- 需求与验收见 `versions/01-需求文档.md` / `versions/02-项目验收单.md`

| 版本 | 日期 | 里程碑 | 说明 |
|---|---|---|---|
| 0.4.3 | 2026-07-16 | 派生反哺(Q16-M1) | V2.1.2~V2.1.5 的五文件框架扩展审阅后转正:Registry source 溯源/Changed、Parser 多段指令名、Bus 可取消、DockingHost 比例语义修正、help 分组展示(明细见模板 TEMPLATE_VERSION.md) |
| 0.4.2 | 2026-07-15 | 派生反哺 | 由 OneHistoryStudio 回灌:①控制台多行日志逐行拆分 + 滚动单位 Item→Pixel,修复底部长日志显示不全;②CommandRegistry 新增 Unregister(模块热重载下线所需) |
| 0.4.1-M4 | 2026-07-12 | M4 修补 | 修复 x64 回收站删除闪退(SHFILEOPSTRUCT 误用 Pack=1,详见下方 M4 要点);资源窗口新增“打开文件夹…”与“恢复默认工作区”(res.root 的图形入口) |
| 0.4.0-M4 | 2026-07-12 | M4 | 控制窗口群(JSON 面板)+ 资源窗口 + res.*/panel.* 指令组。验收 5 / 6 达成;R-06 越界拒绝实测。二次开发权限规则见 meta/二次开发演进手册.md |
...
- **二次开发注意**:冻结区 / 谨慎区 / 自由区与上报规则见 `meta/二次开发演进手册.md`,
  接手前必读。
