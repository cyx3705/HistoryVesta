...
源码当前进入 `3.0.1` 默认最小能力整改候选，版本线不再与 OneHistoryStudio 对齐，`0.7.x` 仅保留用于回滚。
...
.\b-Code-AppShell\eng\Publish-AppShell.ps1 -Version 3.0.1

# 仅在审核通过、代码和消费文档均已提交且干净后执行
.\b-Code-AppShell\eng\Publish-AppShell.ps1 -Version 3.0.1 -Publish
...
审核阶段的消费方必须临时指向
`b-Publish/staging/3.0.1/packages`；正式提升后改用
