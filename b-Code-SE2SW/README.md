...
- [V3.6.5：识别链路修复与人工会话验收](docs/41-V3.6.5-识别链路修复与人工会话验收.md) —— 复用判据纳入识别开关、
  落盘闸门按请求覆盖、缺件生成装配体、会话崩溃改用专属进程；真机四轮复现 42/53 完全识别、330 特征、草图 140/140
- [V3.6.4：FeatureWorks 会话激活态](docs/40-V3.6.4-FeatureWorks会话激活态.md) —— **根因结案**：自动识别要求该
  SolidWorks 进程被人工识别激活过一次，未激活时静默返回 0；`SetAdvancedOptions` 的返回值是一次调用即可判定的探针。
  含证据矩阵、停止 `UnloadAddIn` 销毁激活态、首件即定案的响亮失败与哑零件降级
