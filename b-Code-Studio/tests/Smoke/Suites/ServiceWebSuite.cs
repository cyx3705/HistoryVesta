...
internal static class ServiceWebSuite
{
    ...
    private static async Task VerifyServiceCompositionAsync()
    {
        ...
        try
        {
            ...
            SmokeKit.Equal(133, composition.Registry.All().Count, "service command count");
            foreach (var name in new[]
                     {
                         "help", "history", "run", "res.list", "db.query", "proj.list",
            ...
            SmokeKit.True(
                StudioCommandDataDeserializer.Deserialize("command.list", catalogJson)
                is List<AppShell.Shell.Mcp.CommandCatalogRow> rows && rows.Count == 133,
            ...
            SmokeKit.True(
                remoteCatalog.Data is List<AppShell.Shell.Mcp.CommandCatalogRow> remoteRows
                && remoteRows.Count == 133,
                "HTTP command catalog typed projection");
            var remoteData = new RemoteDataService(client);
            SmokeKit.True(remoteData.ListConnections().Contains("main"), "remote IDataService connections");
            SmokeKit.True(remoteData.ListTables("main").Count > 0, "remote IDataService tables");
        }
        finally
        {
            composition.Dispose();
