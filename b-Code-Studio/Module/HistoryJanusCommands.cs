...
public sealed class HistoryJanusCommands
{
    ...
    public string Status()
    {
        var version = typeof(HistoryJanusCommands).Assembly
            ...
            ?? "unknown";
        return $"HistoryJanus {version} 已由 HistoryVulcan 加载;已注册项目总览、项目操作和 github 三个页面;分支历史内嵌于项目操作并与 Git 文件规则同级切换;Meta 文件夹已合并到项目总览;github.* 只读命令已并入,写操作仅限页面;业务命令已接入宿主命令总线";
    }
}
