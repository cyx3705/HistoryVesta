...
public sealed class FreezeBlockerTests
{
    ...
    public async Task WebGatewayRejectsRequestBodiesOverOneMiBBeforeDeserialization()
    {
        ...
        using (gateway)
        {
            Assert.True(gateway.Start(FreePort()).Success);
            using var client = new HttpClient { BaseAddress = new Uri($"http://127.0.0.1:{gateway.Port}/") };
            client.DefaultRequestHeaders.Add("X-AppShell-Client", "Shell");
    ...
    {
        var name = $"Local\\AppShell.Tests.{Guid.NewGuid():N}";
        using var owner = new Mutex(initiallyOwned: true, name, out var created);
        Assert.True(created);
        var method = typeof(AppShell.ServiceHost.ServiceHost).GetMethod(
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "appshell-history-security-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    var log = new MemoryLog();
                    var bus = new CommandBus(registry, log);
                    _ = new AppShell.Shell.Console.ConsoleView(log, bus, history);
            ...
            Assert.Contains("# AppShell.CommandHistory.v2:redacted", persisted, StringComparison.Ordinal);
