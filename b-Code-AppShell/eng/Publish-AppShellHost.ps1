...
$formalRoot = Join-Path $repoRoot 'z-Package-AppShell'
...
function Assert-HostDirectory {
    param([string]$Path)
    $exe = Join-Path $Path 'AppShell.exe'
    if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) {
        throw "Host snapshot is missing AppShell.exe: $Path"
    }
...
function Assert-Snapshot {
    ...
    foreach ($required in @('AppShell.reuse.md', 'README.md', 'manifest.json', 'SHA256SUMS')) {
    ...
    if ([string]$manifest.version -ne $Version -or [string]$manifest.product -ne 'AppShell') {
        throw 'Snapshot manifest identity does not match the requested AppShell version'
    }
...
try {
    ...
    [IO.File]::WriteAllText(
        (Join-Path $temporary 'AppShell.reuse.md'),
...
- `host/AppShell.exe`: win-x64, framework-dependent AppShell host.
- `docs/`: current API, module, runtime, change, and UI style contracts.
- `AppShell.reuse.md`: minimal entry point for projects and AI consumers.
- `manifest.json` and `SHA256SUMS`: snapshot identity and integrity.

Run `host/AppShell.exe`. Historical releases are stored under `b-Publish/history/`.
"@
    [IO.File]::WriteAllText(
    ...
    $releaseInputs = @('b-Code-AppShell', 'b-Office/package', 'project.manifest.json')
    ...
    Write-Host "Prepared AppShell $Version host snapshot at $candidateRoot"
    ...
    if ($DeployToZ) {
        ...
        Write-Host "Deployed AppShell $Version host snapshot to $formalRoot"
    }
}
finally {
